...
class ImageToImage
{
    ...
    public function setSourceImage($image)
    {
        ...
        if (!@preg_match('@\.(JPG|jpg|JPEG|jpeg|PNG|png|GIF|gif)@', $this->sourceImage, $extension)) {
    ...
    {
        $this->sourceImageExtension = strtolower($extension[1]);
    }
    ...
    {
        $imageWidth = $this->sourceImageWidth;
        $imageHeight = $this->sourceImageHeight;

        if ($this->newWidth){
            $imageWidth = $this->newWidth;
        }
        if ($this->newHeight){
            $imageHeight = $this->newHeight;
        }

	    if( !is_string( $positionX ) && !is_string( $positionY ) )
	    {
        	...
        	list( $mark_w, $mark_h ) = getimagesize( $this->waterMark );

            $position = $positionX . '_' . $positionY;
        	...
        	switch( $position ) {
        	    case 'top_left':
            		...
            		break;
        	    case 'top_center':
		            $this->positionX = ( $imageWidth - $mark_w ) / 2;
		            $this->positionY = 0;
		            break;
	            case 'top_right':
		            $this->positionX = $imageWidth - $mark_w;
		            $this->positionY = 0;
		            break;
	            case 'middle_left':
		            $this->positionX = 0;
		            $this->positionY = ( $imageHeight / 2 ) - ( $mark_h / 2 );
		            break;
	            case 'middle_center':
		            $this->positionX = ( $imageWidth - $mark_w ) / 2;
		            $this->positionY = ( $imageHeight / 2 ) - ( $mark_h / 2 );
	            ...
	            case 'bottom_left':
		            $this->positionX = 0;
		            $this->positionY = $imageHeight - $mark_h;
		            break;
	            case 'bottom_center':
		            $this->positionX = ( $imageWidth - $mark_w ) / 2;
		            $this->positionY = $imageHeight - $mark_h;
    ...
    public function create($filename = null,$quality)
    {
        ...
        switch ($extension) {
        case 'jpeg':
            imagejpeg($thumbnail, $filename, $quality);
            break;
        case 'gif':
            imagegif($thumbnail, $filename, $quality);
            break;
        case 'png':
            imagepng($thumbnail, $filename, $quality);
        default:
            break;
        }
